...
if(is_array($arResult["DETAIL_PICTURE"])) {
	...
	//DETAIL_IMG//
	if ($arParams['USE_BX_RESIZE_IMAGE'] === 'Y') {
...
if(is_array($arResult["MORE_PHOTO"]) && count($arResult["MORE_PHOTO"]) > 0) {
	...
	foreach($arResult["MORE_PHOTO"] as $key => $arFile) {
		...
		if ($arParams['USE_BX_RESIZE_IMAGE'] === 'Y') {
            ...
            $arFile['WIDTH'] = $arParams["DISPLAY_MORE_PHOTO_WIDTH"] ? $arParams["DISPLAY_MORE_PHOTO_WIDTH"] : 86;
            $arFile['HEIGHT'] = "auto";
...
//HIGHLOAD ADVANTAGES//
$arHighloadProperty = $arResult["PROPERTIES"]['advantage_1'];
...
if ( Loader::IncludeModule('highloadblock') && !empty($sTableName) && !empty($arHighloadProperty["VALUE"]) )
{
	...
	if($hlblock)
    {
		...
		{	
		  ...
		  $arResult["PROPERTIES"]['advantage_1']['EXTRA_VALUE'][] = $arRecord;
