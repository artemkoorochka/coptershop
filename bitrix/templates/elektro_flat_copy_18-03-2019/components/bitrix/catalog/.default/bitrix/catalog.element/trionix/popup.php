...
if($request->isPost() && check_bitrix_sessid()) {
	...
	$arParams = $request->getPost("arParams");
	 if(!empty($arParams))//Ниже строка закоментирована так как там возникает ошибка.
	...
	switch($action) {		
		...
		case "test_form":
			//CREDIT_CLICK//
			$APPLICATION->IncludeComponent("altop:multi_select_bank.form.click", "multi_select_bank", 
				array(																
					"ELEMENT_ID" => $elementId,
					"ELEMENT_AREA_ID" => $elementAreaId,
					"USE_FILE_FIELD" => $arParams["1CB_USE_FILE_FIELD"],
