...
/*global $APPLICATION;
$APPLICATION->SetAdditionalCss("/bitrix/js/popular.categories/slick/slick.css");
$APPLICATION->AddHeadScript("/bitrix/js/popular.categories/slick/slick.js");*/
...
Asset::getInstance()->addCss($baseUrl."/bitrix/js/popular.categories/slick/slick.css");
Asset::getInstance()->addCss($baseUrl."/bitrix/js/popular.categories/slick/slick-theme.css");
Asset::getInstance()->addJs($baseUrl."/bitrix/js/popular.categories/slick/slick.js");
...
// if ($USER->GetID() == 8007) {
	...
	?>
	<script>
		var arrItem = <?=json_encode($arrItem)?>;
...
global $USER;
//echo "ID user: ".$USER->GetID()."</br>";
$arGroupAvalaible = array(8,13);//groups: opt,opt_nal)
$arGroups = CUser::GetUserGroup($USER->GetID());
$result_intersect = array_intersect($arGroupAvalaible, $arGroups);

$rsStore = CCatalogStoreProduct::GetList(array(), array('PRODUCT_ID' => $arResult["ID"], 'STORE_ID' => 1), false, false, array());//array('AMOUNT')
$arStore = $rsStore->Fetch();
//echo "<pre>"; print_r($arStore); echo "</pre>";

$db_res_opt = CPrice::GetList(array(),array("PRODUCT_ID" => $arResult["ID"], "CATALOG_GROUP_ID" => 3));//opt
$ar_res_opt = $db_res_opt->Fetch();
//echo "<pre>"; print_r($ar_res_opt["PRICE"]); echo "</pre>";
$arr_num = explode('.',$ar_res_opt["PRICE"]);
//echo "<pre>"; print_r($arr_num); echo "</pre>";
$arr_num_opt = mb_strlen($arr_num[0]);
if($arr_num_opt >= 6){ $price_current_style = "style='font-size:21px !important;'"; }
$db_res_nal = CPrice::GetList(array(),array("PRODUCT_ID" => $arResult["ID"], "CATALOG_GROUP_ID" => 6));//nal
$ar_res_nal = $db_res_nal->Fetch();
//echo "<pre>"; print_r($ar_res_nal); echo "</pre>";
$str_price_nal = FormatCurrency($ar_res_nal["PRICE"],$ar_res_nal["CURRENCY"]);
if(!empty($result_intersect)){
	//echo 'group opt'."</br>";
	$MAX_QUANTITY = $arStore["AMOUNT"];
}else{
	//echo 'no group opt'."</br>";
...
<?
/*echo '<pre>';
var_dump($arResult["PROPERTIES"]['advantage_1']['EXTRA_VALUE']);
echo '</pre>';*/
/*
foreach($arResult["PROPERTIES"]['advantage_1']['EXTRA_VALUE'] as $item){
	echo $item['UF_NAME'].' : /upload'.$item['UF_FILE']['SUBDIR'].'/'.$item['UF_FILE']['FILE_NAME'].'<br>';
}*/
?>
...
if(array_key_exists("PROPERTIES", $arResult) && is_array($arResult["PROPERTIES"])) {
	//installment//
	if(array_key_exists("installment", $arResult["PROPERTIES"]) && $arResult["PROPERTIES"]["installment"]["VALUE"] == 'Да'){
		$sticker .= "<span class='hit installment_s'>".GetMessage("CATALOG_ELEMENT_INSTALLMENT")."</span>
		<div style='display:none;'>".$arResult["PROPERTIES"]["installment"]["VALUE"]."</div>";	
	   // echo "<div style='display:none;'>".$arResult["PROPERTIES"]["installment"]["VALUE"]."</div>";
	}
	//NEW//
	if(array_key_exists("NEWPRODUCT", $arResult["PROPERTIES"]) && !$arResult["PROPERTIES"]["NEWPRODUCT"]["VALUE"] == false)
		$sticker .= "<span class='new'>".GetMessage("CATALOG_ELEMENT_NEWPRODUCT1")."</span>";
	//HIT//
...
<div id="<?=$arItemIDs['ID']?>" class="catalog-detail-element" itemscope itemtype="http://schema.org/Product">
		...
		<div class="column first<?=($arResult["COLLECTION"]["THIS"]) ? " colletion" : ""?>">
					...
					<div class="clr"></div>
						...
						<!--<ul>-->
							...
							if(count($arResult["MORE_PHOTO"]) > 0) {
								?>
								<div class="slider_elem_images_wrapper">
								<?
			...
			<?
					...
					if(!empty($arResult["PROPERTIES"]["GIFT"]["FULL_VALUE"])) {?>
						...
						<div class="ribbon"><span>gift</span></div>
		...
		<div class="column second">			
			<div class="catalog-detail">
				<?if(!$arResult["COLLECTION"]["THIS"]) {?>
					<div class="article_rating padding-left-0">
							...
							if(isset($arResult["OFFERS"]) && !empty($arResult["OFFERS"]) && $arSetting["OFFERS_VIEW"]["VALUE"] != "LIST") {
								foreach($arResult["OFFERS"] as $key => $arOffer) {?>
									<div id="article_<?=$arItemIDs['ID'].'_'.$arOffer['ID']?>" class="article<?=($key == $arResult['OFFERS_SELECTED'] ? '' : ' hidden');?>">
										<?=GetMessage("CATALOG_ELEMENT_ARTNUMBER")?><?=!empty($arOffer["PROPERTIES"]["CML2_ARTICLE"]["VALUE"]) ? $arOffer["PROPERTIES"]["CML2_ARTICLE"]["VALUE"] : "-";?><?//ARTNUMBER?>
									</div>
								<?}
							//DETAIL_ARTICLE//
							} else {?>
								<div class="article">
									<?=GetMessage("CATALOG_ELEMENT_ARTNUMBER")?><?=!empty($arResult["PROPERTIES"]["CML2_ARTICLE"]["VALUE"]) ? $arResult["PROPERTIES"]["CML2_ARTICLE"]["VALUE"] : "-";?><?//ARTNUMBER?>
								</div>
							<?}?>
						</div>
						<?//DETAIL_RATING//?>
					...
					if(!empty($arResult["PREVIEW_TEXT"])) {?>				
						<div class="catalog-detail-preview-text padding-left-0" itemprop="description">
							<?=$arResult["PREVIEW_TEXT"]?>
						</div>
					<?}?>
					<?// ADVANTAGES //?>
					<?if(!empty($arResult["PROPERTIES"]['advantage_1']['EXTRA_VALUE'])){?> 
						<div class="advantages-hblock" style="clear: both">
						<h3><?=GetMessage('CATALOG_ELEMENT_ADVANTAGES');?></h3>
						<div class="advantages-hwrapper">
						<? foreach($arResult["PROPERTIES"]['advantage_1']['EXTRA_VALUE'] as $item){?>
							<div class="advantages-hwrapper-item">
								<div class="adv-icon">
								<img src="/upload/<?=$item['UF_FILE']['SUBDIR'];?>/<?=$item['UF_FILE']['FILE_NAME'];?>"/>
								</div>
								<div class="adv-name"><?=$item['UF_NAME'];?></div>
							</div>
						<?}?>
						</div>
					</div>	
				...
				<div class="column three<?=($arResult["COLLECTION"]["THIS"]) ? " colletion" : ""?>">
					<div class="price_buy_detail" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
						...
						<div class="catalog-detail-price" id="<?=$arItemIDs['PRICE'];?>">
										...
										<div id="detail_price_<?=$arItemIDs['ID'].'_'.$arOffer['ID']?>" class="detail_price<?=($key == $arResult['OFFERS_SELECTED'] ? '' : ' hidden');?>">											
											...
											<?} else {
												...
												<span class="catalog-detail-item-price">
													<span <?=$price_current_style?> class="catalog-detail-item-price-current">
														...
														<span>
															...
															if($arOffer["CHECK_QUANTITY"] && $inProductQnt){
																...
																if ($arOffer["CATALOG_QUANTITY"] < 5) {
																	echo GetMessage("NAL_MALO");
																}

																if ($arOffer["CATALOG_QUANTITY"] >= 5) {
										     						echo GetMessage("NAL_NORM");
										     					}
												...
												<div class="avl">
													...
													<span>
														...
														if($arResult["TOTAL_OFFERS"]["QUANTITY"] > 0 && $inProductQnt){
															if ($arResult["TOTAL_OFFERS"]["QUANTITY"] < 5) {
																echo GetMessage("NAL_MALO");
															}
							...
							//DETAIL_PRICE//
								...
								<?} else {
									...
									<?}?>
									<? if(!empty($result_intersect))://если группа оптовик?>
									<span class="catalog-detail-item-price">
										<span <?=$price_current_style?> class="catalog-detail-item-price-current">
											<?//=$ar_res_opt["PRICE"];?>
										...
										<?if(!empty($str_price_nal)):?>
										<span <?=$price_current_style?> class="catalog-detail-item-price-current">
											<a style="color:#000;text-decoration: none;" title="Оформите заказ, выберите оплата наличными и менеджер исправит цену в заказе."><?=$str_price_nal;//для оптовиков?></a>
										</span>
										<span class="unit">
											<? echo "опт.нал.";?>
										</span>
								...
								<div class="available">
									<?if($arResult["CAN_BUY"]) {?>
										<?if(!empty($result_intersect) && $arStore['AMOUNT'] <= 0){//для оптовиков?>
											<meta content="OutOfStock" itemprop="availability" />
											<div class="not_avl">
												<i class="fa fa-times-circle"></i>
												<span><?=GetMessage("CATALOG_ELEMENT_NOT_AVAILABLE")?></span>
											...
											<meta content="InStock" itemprop="availability" />
												...
												<? if(!empty($result_intersect))://если группа оптовик?>
													<?=GetMessage("CATALOG_ELEMENT_AVAILABLE");
														...
														//echo " ".$arStore['AMOUNT'];//для оптовиков?>
												<? else: ?>
													<?=GetMessage("CATALOG_ELEMENT_AVAILABLE");
									...
									<?if($arResult["CAN_BUY"]) {
										...
										<?} else {?>
											<form action="<?=SITE_DIR?>ajax/add2basket.php" class="add2basket_form">
												...
												<?}?>
												<?if(!empty($result_intersect) && $arStore['AMOUNT'] <= 0):?>
												<?else:?>
												<?if(!$arResult["COLLECTION"]["THIS"]) {?>
												...
												<?endif?>
													...
													<?//TINKOFFBANK?>
													<?
														//echo "<pre>"; print_r($arResult["ITEM_PRICES"][0]["PRICE"]); echo "</pre>";
														$credit_price = $arResult["ITEM_PRICES"][0]["PRICE"];
														$credit_sum = ceil($credit_price/12);
														$credit_sum_all = number_format($credit_sum, 0, '', ' ');
														//echo "<pre>"; print_r($credit_sum_all); echo "</pre>";
														//echo $arResult["MIN_PRICE"]["MIN_QUANTITY"];
													?>
													<?//target="_blank"?>
													<script>
														// console.log('вавававава');
													</script>
													<form action="https://loans.tinkoff.ru/api/partners/v1/lightweight/create" method="post" id="formCreditElement1">
													<input name="shopId" value="coptertime" type="hidden"/>
													<input name="showcaseId" value="coptertime" type="hidden"/>
													<input name="promoCode" id="itemPromoCode" value="default" type="hidden"/>
													<input name="itemName_0" value="<?=$arResult["NAME"]?>" type="hidden"/>
													<input name="itemQuantity_0" id="itemQuantity_0" value="<?=$arResult["MIN_PRICE"]["MIN_QUANTITY"]?>" type="hidden"/>
													<input name="itemPrice_0" id="itemPrice_0" value="<?=$credit_price?>.00" type="hidden"/>
													<input name="itemCategory_0" value="<?=$arResult["NAME"]?>" type="hidden"/>
													<input name="sum" id="all_sum" value="<?=$credit_price?>.00" type="hidden">
													<input name="customerEmail" id="customerEmail" value="" type="hidden"/>
													<input name="customerPhone" id="customerPhone" value="" type="hidden"/>
													</form>
													<?if(!empty($result_intersect) && $arStore['AMOUNT'] <= 0):?>
													<?else:?>
													<? if($arResult["PROPERTIES"]["installment"]["VALUE"] == 'Да') { ?>
													<button style="padding:10px 0;" class="btn_buy btn_green" id="<?=$arItemIDs['POPUP_BTN']?>" data-action="test_form"><?=GetMessage("CATALOG_CREDIT_BUTTON1")."&nbsp;".$credit_sum_all."&nbsp;".GetMessage("CATALOG_CREDIT_BUTTON2")?></button>
													<? } ?>
													<?endif?>
													<?//END TINKOFFBANK?>											
						...
						<?//DETAIL_SUBSCRIBE//?>
						<?if(!empty($result_intersect))://для оптовиков?>
						<div id="<?=$arItemIDs['SUBSCRIBE']?>">
							...
							<?	
								if(isset($arResult["JS_OFFERS"]) && !empty($arResult["JS_OFFERS"])):
									...
									if(!$arResult["CAN_BUY"] && $arResult["CATALOG_SUBSCRIBE"] == 'Y' or $arStore["AMOUNT"] <= 0)://для оптовиков?>
										<div id="catalog-subscribe-from" class="catalog-subscribe-from">
											<?$APPLICATION->includeComponent("bitrix:catalog.product.subscribe", "",
												array(
	...
	<div class="tabs-wrap tabs-catalog-detail">
		...
		if(!$arResult["COLLECTION"]["THIS"] && (!empty($arResult["DISPLAY_PROPERTIES"]) || $strMainOffersProps)) {?>
			<div class="tabs__box">					
				<div id="<?=$arItemIDs['PROPERTIES']?>">
					<div class="catalog-detail-properties">								
						<?
                        // <editor-fold defaultstate="collapsed" desc=" # Проверка - существуют ли в группах свойства с незаполненными значениями">
                        $groopenCount = 1;
                        while (!empty($arParams['GROUPPEN_TITLE_' . $groopenCount])){
                            $isEmpty = true;
                            foreach ($arParams["GROUPPEN_PROPERTIES_" . $groopenCount] as $code){
                                if(
                                    is_array($arResult["PROPERTIES"][$code]) &&
                                    !empty($arResult["PROPERTIES"][$code]["VALUE"])
                                ){
                                    $isEmpty = false;
                                    break;
                                }
                            ...
                            if($isEmpty){
                                unset($arParams['GROUPPEN_TITLE_' . $groopenCount]);
                                unset($arParams['GROUPPEN_PROPERTIES_' . $groopenCount]);
                            }
                        ...
                        // <editor-fold defaultstate="collapsed" desc=" # Вывод характеристик, расскиданных по группам">
                        $groopenCount = 1;
                        while (!empty($arParams['GROUPPEN_TITLE_' . $groopenCount])){
                            ?>
                            <h3><?=$arParams['GROUPPEN_TITLE_' . $groopenCount]?></h3>
                            <?
                            foreach ($arParams["GROUPPEN_PROPERTIES_" . $groopenCount] as $code){
                                if(is_array($arResult["DISPLAY_PROPERTIES"][$code])){
                                    $v = $arResult["DISPLAY_PROPERTIES"][$code];
                                    ?>
                                    <div class="catalog-detail-property">
                                        <div class="name"><?=$v["NAME"]?></div>
                                        <?if(!empty($v["FILTER_HINT"])) {?>
                                ...
                                elseif(
                                    ...
                                    <div class="catalog-detail-property">
                                        ...
                                        <?}?>
                                        <div class="dots"></div>
                                        <div class="val"><?=is_array($v["VALUE"]) ? implode(", ", $v["VALUE"]) : $v["VALUE"];?></div>
                                    </div>
                                    <?
                                }
                        ...
                        // <editor-fold defaultstate="collapsed" desc=" # Вывод дополнительных характеристик">
                        if($groopenCount > 1):
                            ?>
                            <h3><?=$arParams['GROUPPEN_TITLE_ATHER']?></h3>
                        <?
                        endif;
		...
		//ACCESSORIES_TAB//
		if(!empty($arResult["PROPERTIES"]["ACCESSORIES"]["VALUE"])) {?>
			<div class="tabs__box custom_accessories" id="accessories-to">
				<?if($arParams["AJAX_OPTION_HISTORY"] == "Y" && $arParams["AJAX_MODE"] == "N") { ?>
...
<div class="sect__text"><span>Тип устройства</span></div>
...
<?
	...
	$arAcsPrFilter = array("ID" => $arResult["PROPERTY_ACCESSORIES_ID"]);
	//echo "<pre>"; print_r($arAcsPrFilter); echo "</pre>";
	
	foreach($arAcsPrFilter["ID"] as $key_id){
		$res_id = CIBlockElement::GetByID($key_id);
		
		if($arr_res = $res_id->GetNext()){
			$arr_id = CIBlockElement::GetProperty($arr_res["IBLOCK_ID"], $arr_res["ID"], array("sort" => "asc"), Array("CODE"=>"type"));
			
			while ($arr_ob = $arr_id->GetNext()){
				
				echo "<div id='data_id' data-id='".$arr_ob['VALUE']."'></div>";
				//echo "<pre>"; print_r($arr_ids); echo "</pre>";
			}
			
		}
	}
	
	echo "<label for='' id='all'>Все&nbsp;<span id='count_all'></span></label>";

	foreach($arAcsPrFilter["ID"] as $key){
		
	$res_k = CIBlockElement::GetByID($key);
	
	//echo "<pre>"; print_r($res_k); echo "</pre>";

		if($ar_res = $res_k->GetNext()){
		$res = CIBlockElement::GetProperty($ar_res["IBLOCK_ID"], $ar_res["ID"], array("sort" => "asc"), Array("CODE"=>"type"));
			
			while ($ob = $res->GetNext()){

				$prop_id = $ob['VALUE'];
				$prop_name = $ob['VALUE_ENUM'];

				
				echo "<input id='checkbox' type='checkbox' value='".$prop_id."' /><label for='".$prop_id."' id='".$prop_id."'>".$prop_name."&nbsp;<span id='count'></span></label>";
...
<?
$array_filter = explode(",", $_GET["idx"]);
$arAcsPrFilter = array(
	"ID" => $arResult["PROPERTY_ACCESSORIES_ID"],
	"PROPERTY_type" => $array_filter,
);
?>	
<?
							$APPLICATION->IncludeComponent("bitrix:catalog.section", "filtered",
								array(
									...
									"ELEMENT_SORT_FIELD2" => "",
									"ELEMENT_SORT_ORDER2" => "",
									"PROPERTY_CODE" => $arResult["PROPERTY_ACCESSORIES_ID"],//new
									"SET_META_KEYWORDS" => "N",		
									"SET_META_DESCRIPTION" => "N",		
									"SET_BROWSER_TITLE" => "N",
									"SET_LAST_MODIFIED" => "N",
									...
									"FILE_404" => "",
									"DISPLAY_COMPARE" => $arParams["DISPLAY_COMPARE"] == 1 ? "Y" : "N",
									"PAGE_ELEMENT_COUNT" => (isset($arParams['NUMBER_ACCESSORIES'])? $arParams['NUMBER_ACCESSORIES']: "12"),//new
									"LINE_ELEMENT_COUNT" => "",
									"PRICE_CODE" => $arParams["PRICE_CODE"],
									"USE_PRICE_COUNT" => $arParams["USE_PRICE_COUNT"] == 1 ? "Y" : "N",
									"SHOW_PRICE_COUNT" => $arParams["SHOW_PRICE_COUNT"],
									...
									"PRODUCT_PROPERTIES" => $arParams["PRODUCT_PROPERTIES"],
									"DISPLAY_TOP_PAGER" => "N",
									"DISPLAY_BOTTOM_PAGER" => "Y",//new
									"PAGER_TITLE" => "",
									"PAGER_SHOW_ALWAYS" => "Y",//new
									"PAGER_TEMPLATE" => "arrows",//new
									"PAGER_DESC_NUMBERING" => "N",
									...
									"PROPERTY_CODE_MOD" => $arParams["PROPERTY_CODE_MOD"],
									"AJAX_MODE" => "Y",//new
									"AJAX_OPTION_JUMP" => "N",//new
									"AJAX_OPTION_HISTORY" => "Y"//new
								),
								$component,
								array("HIDE_ICONS" => "Y")
							);?>
...
<?php
$arSets = CCatalogProductSet::getAllSetsByProduct($arResult['ID'], 2);
...
if($arSet!=NULL){
	...
	$set_quantity=array();
	$set_sort = array();
	$arSetItems = $arSet['ITEMS'];

foreach($arSetItems as $key){
	array_push($set_str,$key['ITEM_ID']);
	$set_quantity[$key['ITEM_ID']] = $key['QUANTITY'];
	$set_sort[$key['ITEM_ID']] = $key['SORT'];
}

	$arOrder = Array('NAME' => 'ASC');
	$arSelect = Array("ID", "NAME", "DATE_ACTIVE_FROM","PREVIEW_PICTURE","DETAIL_PAGE_URL","DETAIL_PICTURE");
	$filter = Array("ID" => $set_str);
	$res = CIBlockElement::GetList($arOrder, $filter, false, Array("nTopCount"=>20),$arSelect);

	$fields = array();

while($ob = $res->GetNextElement()){
	$arFields = $ob->GetFields();
	array_push($fields, $arFields); 
}
	$arCount = count($fields);

for($i=0;$i<$arCount;$i++){
	$fields[$i]['SORT'] = $set_sort[$fields[$i]['ID']];
}
	$sort  = array_column($fields, 'SORT');
	$name  = array_column($fields, 'NAME');
	array_multisort($sort,SORT_ASC,$name,SORT_ASC,$fields);
?>
<div class="custom-set-wrapper" style="display:none;">
<div class="custom-set">
<div class="h3"><?=GetMessage("CATALOG_ELEMENT_RECOMMEND_TO_BUY");?></div>
<?
	$counter=0;
	...
	foreach($fields as $field){
		...
		$elemQuantity = CCatalogProduct::GetByID($field['ID']);	
		
		if($elemQuantity['QUANTITY']==0)
				continue;
		$counter++;
		
		$strMainID_cust = $this->GetEditAreaId($field['ID']);
		$arItemIDs_cust = $strMainID_cust;
	?>
	
	<div class="catalog-item added-item"> 
		<div class="catalog-item-info">
			<div class="catalog-item-image-cont">
				<div class="catalog-item-image">
					<a href="<?=$field['DETAIL_PAGE_URL'];?>">
						<? if($field['PREVIEW_PICTURE']):?>
						<img src="<?=$arFileTmp = CFile::ResizeImageGet(
			...
			<div class="form_wrapper_sets">
			<form action="/ajax/add2basket.php" class="add2basket_form custom_class_basket" method="post" onsubmit="">
				<input type="hidden" name="ID" class="id" value="<?=$field['ID']?>" />
				<input type="text" id="quantity_<?=$arItemIDs_cust;?>" name="quantity" class="quantity" value="<?=$set_quantity[$field['ID']];?>" style="display:none;">	
				<button  type="submit" id="<?=$arItemIDs_cust;?>" class="btn_buy detail btn_custom_buy" name="add2basket">
					<i class="fa fa-shopping-cart"></i>
					<span>
						<?=($arSetting["NAME_BUTTON_TO_CART"]["VALUE"] ? $arSetting["NAME_BUTTON_TO_CART"]["VALUE"] : GetMessage("CATALOG_ELEMENT_ADD_TO_CART"))?>
					</span>
				</button>
			</form>
			</div>
...
<?if(isset($arResult["OFFERS"]) && !empty($arResult["OFFERS"])) {
	...
	$arJSParams = array(
		...
		"PRODUCT" => array(
			...
			"MAX_QUANTITY" => $MAX_QUANTITY,//для оптовиков
...
$arSetting['1CB_REQUIRED_FIELDS'] = $arParams['1CB_REQUIRED_FIELDS'];
...
<script type="text/javascript">	
		$('.slider_elem_images_wrapper').slick({
		 slidesToShow: 4,
		 infinite:false,
		 responsive: [
		 {
	...
	}).css({'overflow':'inherit','height':'auto'});

	$('.slider_elem_images_wrapper').on('breakpoint', function(event, slick, direction){
  console.log('breakpoint');
    ...
    $('.btn_buy.boc_anch').click(function(event) {
        gtag('event', 'Купить в 1 клик в карточке товара', {'event_category': 'buy_one_click_detail_product_category', 'event_action': 'buy_one_click_detail_product_click'});
    });

    $('.btn_buy[data-action="test_form"]').click(function(event) {
        gtag('event', 'В рассрочку', {'event_category': 'buy_installment_category', 'event_action': 'buy_installment_click'});
    });
</script>
